...
describe("Custom Contract test CONSUME type", function () {
  ...
  before(async () => {
    ...
    await pablockToken.requestToken(user.address, 10);
  ...
  it("Should allowed user", async () => {
    const [_, payer, user] = await ethers.getSigners();
    const prevBalance: BigNumber = await pablockToken.balanceOf(payer.address);
    ...
    const nonce = await metaTransaction.getNonce(payer.address);
    ...
    expect(prevBalance.sub(await pablockToken.balanceOf(payer.address))).to.eq(
      ethers.utils.parseEther("1"),
    );
  });
  it("Should increment and user pay token", async () => {
    const [_, payer, user] = await ethers.getSigners();

    const prevBalance: BigNumber = await pablockToken.balanceOf(user.address);
    ...
    expect(prevBalance.sub(await pablockToken.balanceOf(user.address))).to.eq(
      ethers.utils.parseEther("1"),
    );
  });
